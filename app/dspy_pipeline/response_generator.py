...
class ResponseOutput(BaseModel):
    ...
    tone: str = Field(..., description="Tone of the response (e.g., professional_interested, polite_decline)")
    length: int = Field(..., description="Length of response in characters")
    key_points: list[str] = Field(default_factory=list, description="Key points addressed in response")
...
class GenerateResponse(dspy.Signature):
    ...
    is_short_message: str = dspy.InputField(desc="'yes' if message is short/ongoing thread, 'no' otherwise")
    has_calendar_link: str = dspy.InputField(desc="'yes' if message contains Calendly/Ashby/calendar link, 'no' otherwise")
    ...
    # Output
    response: str = dspy.OutputField(desc="""Natural, human response (2-3 sentences for normal, 1 sentence for short messages).
...
class ResponseGenerator:
    ...
    def generate(
        ...
        try:
            ...
            spanish_indicators = ["hola", "gracias", "estoy", "busco", "interesado", "posición", "salario", "¿"]
            ...
            short_responses = ["dale", "buenísimo", "perfecto", "ok", "okay", "genial", "great", "sounds good"]
            ...
            calendar_indicators = ["calendly", "ashby", "calendar", "cal.com", "schedule", "agendar"]
            has_calendar_link = "yes" if any(indicator in message_lower for indicator in calendar_indicators) else "no"
            ...
            result = self.generator(
                ...
                tech_stack=", ".join(analysis.tech_stack) if analysis.tech_stack else "Not specified",
